...
err_t buf_new(size_t capacity, bool auto_compact, buf_t *result);
...
// returns the available write space size (without compactifying)
size_t buf_immediately_available_write_size(buf_t const *self);

// same as above, but takes into account the possibility of compactifying
