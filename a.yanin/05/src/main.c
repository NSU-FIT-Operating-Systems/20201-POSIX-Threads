...
static void print_usage(void) {
    fputs("Usage: thread-cancel-handle\n", stderr);
}

static void child_thread_cleanup(void *) {
    puts("The child thread has been canceled.");
}

static void *child_thread(void *) {
    pthread_cleanup_push(child_thread_cleanup, 0);
